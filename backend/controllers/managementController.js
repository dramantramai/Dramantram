...
// Send Inquiry Email Controller using Resend
export const sendInquiryController = async (req, res) => {
  try {
    const { name, email, phone, referrer, message, services, duration } = req.fields;
    ...
    if (!apiKey) {
      console.warn("WARNING: RESEND_API_KEY is not set in backend/.env");
    ...
    // Format services (which can be a single string or an array depending on submission format)
    let parsedServices = "";
    ...
    // Send the email to Dramantram@gmail.com
    ...
    console.error("Error in sendInquiryController:", error);
    res.status(500).send({
      success: false,
      message: "Error in processing inquiry",
      error: error.message,
    });
  }
};
